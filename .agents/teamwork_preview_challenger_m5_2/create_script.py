    ...
    if (match) {
      ...
      if (prevLevel > 0 && level > prevLevel + 1) {
        issues.push({
          fileName,
          line: i + 1,
          msg: 'Heading level skip: from h' + prevLevel + ' to h' + level + ' (' + title + ')'
        });
      }
      prevLevel = level;
    }
...
function checkTables(content, fileName) {
  ...
  for (const t of tables) {
    ...
    if (!isSep) {
      isValid = false;
      errorMsg = 'Row 2 is not a valid markdown table separator row: ' + t.rows[1].text;
    } else {
      ...
      for (let r = 0; r < t.rows.length; r++) {
        ...
        if (rowCols.length !== colCount) {
          isValid = false;
          errorMsg = 'Row ' + t.rows[r].lineNum + ' column count mismatch: expected ' + colCount + ', got ' + rowCols.length;
          break;
        }
      }
    }
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (match) {
      ...
      callouts.push({
        ...
        type,
        isValid,
        msg: isValid ? 'OK' : 'Invalid alert type: [!' + type + ']. Allowed: ' + validTypes.join(', ')
      });
    }
  }
  return callouts;
...
async function run() {
  ...
  for (const fileName of FILES) {
    ...
    console.log('\\n------------------------------------------------------------');
    console.log('[FILE] ' + fileName + ' (' + content.length + ' chars, ' + blocks.length + ' code blocks)');
    console.log('------------------------------------------------------------');
    ...
    if (headingCheck.issues.length > 0) {
      ...
      for (const issue of headingCheck.issues) {
        console.warn('  [HEADING ISSUE] Line ' + issue.line + ': ' + issue.msg);
      }
    } else {
      console.log('  ✓ Headings: ' + headingCheck.count + ' headings, 0 hierarchy skips');
    }
    ...
    for (const t of tableChecks) {
      if (t.isValid) {
        ...
        console.error('  [TABLE ERROR] Line ' + t.startLine + ': ' + t.errorMsg);
      }
    }
    if (fileTablesFailed === 0) {
      console.log('  ✓ Tables: ' + tableChecks.length + ' tables verified, all valid');
    }
    ...
    for (const c of calloutChecks) {
      if (c.isValid) {
        ...
        console.error('  [CALLOUT ERROR] Line ' + c.line + ': ' + c.msg);
      }
    }
    if (fileCalloutsInvalid === 0) {
      console.log('  ✓ Alert Callouts: ' + calloutChecks.length + ' callouts verified, all valid');
    }
    ...
    if (placeholderChecks.length > 0) {
      ...
      for (const p of placeholderChecks) {
        console.error('  [PLACEHOLDER FOUND] Line ' + p.line + ': ' + p.matched);
      }
    } else {
    ...
    for (const block of blocks) {
      if (block.lang === 'mermaid') {
        ...
        try {
          ...
          console.log('  ✓ Mermaid [Line ' + block.startLine + '-' + block.endLine + '] (' + (parseResult.diagramType || firstLine) + ')');
        } catch (err) {
          ...
          console.error('  ❌ MERMAID ERROR [Line ' + block.startLine + '-' + block.endLine + '] in ' + fileName + ': ' + err.message);
        }
        ...
        try {
          ...
          results.json.passed++;
          console.log('  ✓ JSON [Line ' + block.startLine + '-' + block.endLine + '] (Valid syntax)');
        } catch (err) {
          results.json.failed++;
          const errDetail = {
            fileName,
          ...
          console.error('  ❌ JSON ERROR [Line ' + block.startLine + '-' + block.endLine + '] in ' + fileName + ': ' + err.message);
        }
        ...
        try {
          yamlLoad(block.code);
          results.yaml.passed++;
          console.log('  ✓ YAML [Line ' + block.startLine + '-' + block.endLine + '] (Valid syntax)');
        } catch (err) {
          results.yaml.failed++;
          const errDetail = {
            fileName,
          ...
          console.error('  ❌ YAML ERROR [Line ' + block.startLine + '-' + block.endLine + '] in ' + fileName + ': ' + err.message);
        }
      }
    }
  }
  ...
  console.log('Mermaid Diagrams: ' + results.mermaid.total + ' total | ' + results.mermaid.passed + ' passed | ' + results.mermaid.failed + ' failed');
  console.log('JSON Blocks:      ' + results.json.total + ' total | ' + results.json.passed + ' passed | ' + results.json.failed + ' failed');
  console.log('YAML Blocks:      ' + results.yaml.total + ' total | ' + results.yaml.passed + ' passed | ' + results.yaml.failed + ' failed');
  console.log('Tables:           ' + results.tables.total + ' total | ' + results.tables.passed + ' passed | ' + results.tables.failed + ' failed');
  console.log('Heading Skips:    ' + results.headings.issues + ' issues across ' + results.headings.total + ' headings');
  console.log('Alert Callouts:   ' + results.callouts.total + ' total | ' + results.callouts.valid + ' valid | ' + results.callouts.invalid + ' invalid');
  console.log('Placeholders:     ' + results.placeholders.found + ' forbidden tokens found');
  console.log('================================================================');
...
with open('d:/Idea_DoAn/.agents/teamwork_preview_challenger_m5_2/verify_all_empirical.mjs', 'w', encoding='utf-8') as f:
    f.write(content)
print( Wrote verify_all_empirical.mjs successfully)
