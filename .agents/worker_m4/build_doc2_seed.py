...
100% Full Content, Zero Placeholders, Complete PostgreSQL 16 DDL & Realistic DML Seed Data
"""
import os

target = r'd:\Idea_DoAn\05_Quy_Chuan_&_Test_Cases\Seed_Data_&_Database_Script.md'
os.makedirs(os.path.dirname(target), exist_ok=True)

content = """# 💾 TÀI LIỆU THIẾT KẾ CƠ SỞ DỮ LIỆU & BỘ KỊCH BẢN DỮ LIỆU MẪU (SEED DATA SCRIPT)
...
> **Cơ sở dữ liệu đích:** PostgreSQL 16 (Hỗ trợ Extensions, JSONB, UUID, Partial Indexes, Row-Level Security)  
> **ORM tương thích:** Entity Framework Core 8.0 (Code-First Migrations & Model Snapshot) / Dapper High Performance  
> **Tài liệu tham chiếu:** `PROJECT.md`, `technical_contracts.md`, `Smart_FB_OS_Revised_4members.docx`  
> **Mục tiêu:** Cung cấp toàn văn kịch bản DDL khởi tạo cấu trúc bảng chuẩn hóa 3NF và kịch bản DML nạp dữ liệu mẫu chi tiết, phản ánh đầy đủ 5 thay đổi nghiệp vụ cốt lõi (Dine-in Pre-pay, QR Delivery 20K, Takeaway Staff POS Loyalty 10 ly, WiFi-locked Attendance, Hợp nhất Web Stack).

---

## 📑 MỤC LỤC

1. [TỔNG QUAN KIẾN TRÚC DỮ LIỆU & NGUYÊN TẮC THIẾT KẾ](#1-tổng-quan-kiến-trúc-dữ-liệu--nguyên-tắc-thiết-kế)
2. [KỊCH BẢN DDL POSTGRESQL 16 KHỞI TẠO HỆ THỐNG (FULL SCHEMA)](#2-kịch-bản-ddl-postgresql-16-khởi-tạo-hệ-thống-full-schema)
   - [2.1 Extensions & Kiểu Dữ Liệu ENUM](#21-extensions--kiểu-dữ-liệu-enum)
   - [2.2 Cấu trúc Bảng Chi Nhánh, Mạng WiFi & Bàn Ăn](#22-cấu-trúc-bảng-chi-nhánh-mạng-wifi--bàn-ăn)
   - [2.3 Cấu trúc Bảng Người Dùng, Ca Làm Việc & Chấm Công WiFi](#23-cấu-trúc-bảng-người-dùng-ca-làm-việc--chấm-công-wifi)
   - [2.4 Cấu trúc Bảng Thực Đơn, Biến Thể & Định Lượng Món (BOM)](#24-cấu-trúc-bảng-thực-đơn-biến-thể--định-lượng-món-bom)
   - [2.5 Cấu trúc Bảng Khách Hàng CRM & Sổ Cái Tích Ly (Loyalty 10 Cups)](#25-cấu-trúc-bảng-khách-hàng-crm--sổ-cái-tích-ly-loyalty-10-cups)
   - [2.6 Cấu trúc Bảng Đơn Hàng 3 Loại & Chi Tiết Món (Orders & Items)](#26-cấu-trúc-bảng-đơn-hàng-3-loại--chi-tiết-món-orders--items)
   - [2.7 Cấu trúc Bảng Thanh Toán VietQR / Tiền Mặt & Lịch Sử Giao Dịch](#27-cấu-trúc-bảng-thanh-toán-vietqr--tiền-mặt--lịch-sử-giao-dịch)
   - [2.8 Cấu trúc Bảng Khuyến Mãi, Combo AI, Đánh Giá & Nhật Ký Kiểm Toán](#28-cấu-trúc-bảng-khuyến-mãi-combo-ai-đánh-giá--nhật-ký-kiểm-toán)
3. [KỊCH BẢN DML NẠP DỮ LIỆU MẪU TOÀN DIỆN (FULL SEED DATA SCRIPT)](#3-kịch-bản-dml-nạp-dữ-liệu-mẫu-toàn-diện-full-seed-data-script)
   - [3.1 Dữ liệu Chi Nhánh, Cấu Hình WiFi BSSID/IP & Bàn Ăn](#31-dữ-liệu-chi-nhánh-cấu-hình-wifi-bssidip--bàn-ăn)
   - [3.2 Dữ liệu Người Dùng & Phân Quyền RBAC](#32-dữ-liệu-người-dùng--phân-quyền-rbac)
   - [3.3 Dữ liệu Thực Đơn, Biến Thể, Tuỳ Chọn & Công Thức Pha Chế](#33-dữ-liệu-thực-đơn-biến-thể-tuỳ-chọn--công-thức-pha-chế)
   - [3.4 Dữ liệu Khách Hàng CRM & Tiến Trình Tích Ly (0 Ly, 9 Ly, 10 Ly)](#34-dữ-liệu-khách-hàng-crm--tiến-trình-tích-ly-0-ly-9-ly-10-ly)
   - [3.5 Dữ liệu Mẫu Đơn Hàng Đầy Đủ 3 Loại (DineIn, TakeAway, Delivery)](#35-dữ-liệu-mẫu-đơn-hàng-đầy-đủ-3-loại-dinein-takeaway-delivery)
   - [3.6 Dữ liệu Chấm Công Khóa WiFi & Ca Làm Việc Đối Soát Tiền](#36-dữ-liệu-chấm-công-khóa-wifi--ca-làm-việc-đối-soát-tiền)
4. [HƯỚNG DẪN THỰC THI & TÍCH HỢP EF CORE CODE-FIRST](#4-hướng-dẫn-thực-thi--tích-hợp-ef-core-code-first)

---

## 1. TỔNG QUAN KIẾN TRÚC DỮ LIỆU & NGUYÊN TẮC THIẾT KẾ

Cơ sở dữ liệu Smart F&B OS được thiết kế theo các chuẩn mực doanh nghiệp:
1. **Tính Toàn Vẹn & Nhất Quán (ACID & 3NF)**: Sử dụng khóa chính UUIDv7 / UUIDv4, khóa ngoại với quy tắc `ON DELETE RESTRICT` hoặc `ON DELETE CASCADE` phù hợp, chỉ mục duy nhất (Unique Indexes) ngăn ngừa trùng lặp dữ liệu.
2. **Khớp 100% 5 Hợp Đồng Nghiệp Vụ Cốt Lõi**:
   - `OrderTypeEnum`: Hỗ trợ 3 giá trị cố định `DineIn`, `TakeAway`, `Delivery`.
   - `OrderStatusEnum`: Hỗ trợ `PendingPayment` -> `Paid` -> `Confirmed` -> `Preparing` -> `Ready` -> `Completed` -> `Cancelled`.
   - `BranchWifiConfigs`: Lưu trữ chính xác `wifi_ssid`, `wifi_bssid` (MAC address chuẩn), và `allowed_ip_subnet` (chuẩn CIDR như `192.168.1.0/24`) để xác thực chấm công 2 lớp.
   - `Orders.delivery_fee`: Giá trị mặc định `20000.00` cho đơn `Delivery`.
   - `LoyaltyCupTransactions`: Theo dõi biến động số ly tích lũy và lịch sử đổi 1 ly miễn phí.
3. **Auditing Toàn Diện**: Mọi bảng nghiệp vụ đều có các cột `created_at`, `updated_at`, `is_deleted` (Soft Delete) và bảng `audit_logs` lưu trữ mọi hành vi thay đổi dữ liệu nhạy cảm.

---

## 2. KỊCH BẢN DDL POSTGRESQL 16 KHỞI TẠO HỆ THỐNG (FULL SCHEMA)
...
## 3. KỊCH BẢN DML NẠP DỮ LIỆU MẪU TOÀN DIỆN (FULL SEED DATA SCRIPT)
...
## 4. HƯỚNG DẪN THỰC THI & TÍCH HỢP EF CORE CODE-FIRST

### 4.1 Thực thi qua lệnh PostgreSQL CLI (`psql`)
```bash
# Tạo Database nếu chưa có
psql -U postgres -c "CREATE DATABASE smartfb_db;"

# Chạy DDL khởi tạo schema
psql -U postgres -d smartfb_db -f 01_schema_ddl.sql

# Chạy DML nạp dữ liệu mẫu
psql -U postgres -d smartfb_db -f 02_seed_dml.sql
```

### 4.2 Tự động Seeding trong Entity Framework Core 8.0 (.NET 8)
Trong lớp `SmartFbDbContext.cs`, phương thức `OnModelCreating(ModelBuilder modelBuilder)` hoặc lớp `DbInitializer.cs` được cấu hình nạp seed data tự động khi chạy `dotnet ef database update`:

```csharp
public static class DbInitializer
{
    public static async Task SeedAsync(SmartFbDbContext context)
    {
        if (!context.Branches.Any())
        {
            // Seed Branches & WiFi configs
            var q1Branch = new Branch
            {
                Id = Guid.Parse("b1000000-0000-0000-0000-000000000001"),
                Code = "BR-Q1",
                Name = "Smart Coffee - Quận 1 Flagship",
                Address = "72 Lê Thánh Tôn, P. Bến Nghé, Quận 1, TP.HCM",
                Phone = "02838221101",
                IsActive = true
            };
            context.Branches.Add(q1Branch);
            
            context.BranchWifiConfigs.Add(new BranchWifiConfig
            {
                Id = Guid.Parse("w1000000-0000-0000-0000-000000000001"),
                BranchId = q1Branch.Id,
                WifiSsid = "SmartCoffee_Q1",
